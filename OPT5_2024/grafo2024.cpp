...
void GRAFO::dfs_num(unsigned i, const vector<LA_nodo> &L, vector<bool> &visitado, vector<unsigned> &prenum, unsigned &prenum_ind, vector<unsigned> &postnum, unsigned &postnum_ind) //Recorrido en profundidad recursivo con recorridos enum y postnum
{
	visitado[i] = true;
...
void GRAFO::RecorridoAmplitud() { //Construye un recorrido en amplitud desde un nodo inicial
  ...
  std::cout << "Vamos a construir un recorrido en amplitud" << endl;
  ...
  // 8. Imprimir nodos por distancia
...
void GRAFO::Prim(){
  ...
  while(contador_T != (n - 1)) { // Mientras no hayamos visitado todos los nodos
    ...
    if (comparacion_menor_coste == maxint) {
        std::cout << "El grafo no es conexo. MST incompleto." << endl;
        break;
    }
...
void GRAFO::ComparativaCM(){
 ...
 bool GRAFO::EsConexo() {
    // Vector para marcar los nodos visitados durante el DFS
    vector<bool> visitado(n, false);

    // Vector para almacenar el orden de visita prenum y postnum
    ...
    // Realiza un recorrido en profundidad desde el nodo 0
    dfs_num(0, LS, visitado, prenum, prenum_ind, postnum, postnum_ind);
...
void GRAFO::Kruskal()
{
  ...
  std::vector<AristaPesada> Aristas; // Definimos un vector de AristaPesada llamado Aristas

  // Redimensionamos el vector para que tenga espacio para todas las aristas del grafo
  Aristas.resize(m);

  // Inicializamos un contador en 0 para hacer de indice de aristas
  unsigned indiceArista = 0;
  ...
  for (unsigned i = 0; i < n; i++) {
      ...
      for (unsigned j = 0; j < LS[i].size(); j++) {
         ...
         if (dirigido == 1 || i < LS[i][j].j) {
              ...
              Aristas[indiceArista].extremo1 = i; // Asigna el extremo 1 de la arista actual al nodo actual (i) como su extremo 1 en el vector de aristas.
              Aristas[indiceArista].extremo2 = LS[i][j].j; // Asigna el extremo 2 de la arista actual al nodo destino (j) obtenido de la lista de adyacencia del nodo actual (i).
              Aristas[indiceArista++].peso = LS[i][j].c; // Asigna el peso de la arista actual al campo de peso correspondiente en el vector de aristas.
          }
      }
  }
  ...
  if (!EsConexo()) {
    std::cout << "El grafo no es conexo. No es posible calcular el MST correctamente." << std::endl;
    return;
  }
  ...
  do {
    ...
    if (Raiz[Aristas[head].extremo1] != Raiz[Aristas[head].extremo2]) {
      ...
      std::cout << "Arista numero " << cont << " incorporada (" << Aristas[head].extremo1 + 1;
      std::cout << ", " << Aristas[head].extremo2 + 1 << "), con peso " << Aristas[head].peso << std::endl;
  ...
  std::cout << "El peso del arbol generador de coste minimo es " << pesoMST << "." << std::endl;
}
...
void GRAFO::PDM() {
    ...
    cin >> s;
    s--; // Asume que los nodos se ingresan en base 1
